...
output = cancerdf.groupby(['target']).count()

output1 = output['mean radius']


siri = pd.Series(output1).rename('target')

siri = siri.rename(index={0.0:'malignant', 1.0: 'benign'})

siri = siri.rename_axis(['index'])
...
def answer_two():
    ...
    return siri# Return your answer
...
cancerdf = answer_one()
means = cancerdf.mean()[:-1].values.reshape(1, -1,order = 'C')
print(means)
...
def answer_six():
    cancerdf = answer_one()
    means = cancerdf.mean()[:-1].values.reshape(1, -1,order = 'F')
    model = answer_five()
...
def answer_eight():
    ...
    return Final # Return your answer
